<!DOCTYPE html>
<html>
<head>
	<meta charset="utf-8" />
	<title> HTML에 대하여 </title>
	<link rel="stylesheet" href="css/main.css" />
    <link rel="stylesheet" href="css/community.css" />
    <link rel="stylesheet" href="css/reset.css">
    <link rel="stylesheet" href="css/header.css">
    <link rel="stylesheet" href="css/login.css">
    <link rel="stylesheet" href="css/footer.css">
    
    
    <script type="text/javascript" src="js/jquery-2.1.1.min.js"></script>
	<script type="text/javascript" src="js/jquery-ui.min.js"></script>
	<script type="text/javascript" src="js/jquery.easing.1.3.js"></script>
	<script type="text/javascript" src="js/prefixfree.min.js"></script>
	<script type="text/javascript" src="js/main.js"></script>    
	<script type="text/javascript" src="js/banner1.js"></script>
	<script type="text/javascript" src="js/banner2.js"></script>	
	
</head>
<body>

    <? include_once("header.php"); ?>
    <div class="navi">
        <span><img src="images/home.png" alt="홈" /></span>
        <img src="images/nav_r.png" alt=""/>
        <span>커뮤니티</span>
        <img src="images/nav_r.png" alt=""/>
        <span class="last_navi">질문게시판</span> 
    </div>  
    
    <div class="comunity">
        <div class="community_inner">
            <div class="comunity_header">
                <h2 class="colorChange">공지사항<Br>및 질문게시판</h2>          
            </div>
            
    
        <ul class="tab clearfix">
            <li class="notice"><a href="#">공지사항</a></li>
            <li class="qna"><a href="#">질문게시판</a></li>
        </ul>
        <hr class="notice_line">
           
           
            <div class="borader_box">
                <div class="notice_board">
                    <p class="border_s">
                        <input type="text"><a href="#">검색</a>
                    </p>

                    <table>
                        <colgroup>
                            <col width="200">
                            <col width="200">
                            <col width="600">
                            <col width="200">
                        </colgruop>
                        <thead>
                            <th>글번호</th>
                            <th>작성자</th>
                            <th>글제목</th>
                            <th>작성일</th>
                        </thead>
                        <tbody>
                            <tr>
                                <td>123</td>
                                <td>히히히</td>
                                <td>2016 청소년 정보윤리 창작음악제 참가안내</td>
                                <td>2016.08.08</td>
                            </tr>
                            <tr>
                                <td>123</td>
                                <td>하하하</td>
                                <td>2016 청소년 정보윤리 창작음악제 참가안내</td>
                                <td>2016.08.08</td>
                            </tr>                    
                            <tr>
                                <td>123</td>
                                <td>하하하</td>
                                <td>2016 청소년 정보윤리 창작음악제 참가안내</td>
                                <td>2016.08.08</td>
                            </tr>                    
                            <tr>
                                <td>123</td>
                                <td>하하하</td>
                                <td>2016 청소년 정보윤리 창작음악제 참가안내</td>
                                <td>2016.08.08</td>
                            </tr>
                            <tr>
                                <td>123</td>
                                <td>하하하</td>
                                <td>2016 청소년 정보윤리 창작음악제 참가안내</td>
                                <td>2016.08.08</td>
                            </tr>
                            <tr>
                                <td>123</td>
                                <td>하하하</td>
                                <td>2016 청소년 정보윤리 창작음악제 참가안내</td>
                                <td>2016.08.08</td>
                            </tr>                                                            
                            <tr>
                                <td>123</td>
                                <td>하하하</td>
                                <td>2016 청소년 정보윤리 창작음악제 참가안내</td>
                                <td>2016.08.08</td>
                            </tr>     
                           <tr>
                                <td>123</td>
                                <td>하하하</td>
                                <td>2016 청소년 정보윤리 창작음악제 참가안내</td>
                                <td>2016.08.08</td>
                            </tr>  
                           <tr>
                                <td>123</td>
                                <td>하하하</td>
                                <td>2016 청소년 정보윤리 창작음악제 참가안내</td>
                                <td>2016.08.08</td>
                            </tr>  
                           <tr>
                                <td>123</td>
                                <td>하하하</td>
                                <td>2016 청소년 정보윤리 창작음악제 참가안내</td>
                                <td>2016.08.08</td>
                            </tr>                                                                     
                        </tbody>
                    </table>
                    <div class="paging">
                        <ul class="clearfix">
                            <li><a href="#">1</a></li>
                            <li><a href="#">2</a></li>
                            <li><a href="#">3</a></li>
                            <li><a href="#">4</a></li>
                            <li><a href="#">5</a></li>
                            <li><a href="#">6</a></li>
                            <li><a href="#">7</a></li>
                            <li><a href="#">8</a></li>
                            <li><a href="#">9</a></li>
                            <li><a href="#">10</a></li>
                            <li><a href="#">></a></li>
                            <li><a href="#">>></a></li>
                        </ul>                       
                    </div>

                </div>       
                
                         
                                  
                                           
                                                    
                                                             
                                                                      
                <div class="qna_board">
                    <p class="border_s">
                        <input type="text"><a href="#">검색</a>
                    </p>

                    <table>
                        <colgroup>
                            <col width="200">
                            <col width="200">
                            <col width="600">
                            <col width="200">
                        </colgruop>
                        <thead>
                            <th>글번호</th>
                            <th>작성자</th>
                            <th>글제목</th>
                            <th>작성일</th>
                        </thead>
                        <tbody>
                            <tr>
                                <td>123</td>
                                <td>하하하</td>
                                <td>2016 청소년 정보윤리 창작음악제 참가안내</td>
                                <td>2016.08.08</td>
                            </tr>
                            <tr>
                                <td>123</td>
                                <td>하하하</td>
                                <td>2016 청소년 정보윤리 창작음악제 참가안내</td>
                                <td>2016.08.08</td>
                            </tr>                    
                            <tr>
                                <td>123</td>
                                <td>하하하</td>
                                <td>2016 청소년 정보윤리 창작음악제 참가안내</td>
                                <td>2016.08.08</td>
                            </tr>                    
                            <tr>
                                <td>123</td>
                                <td>하하하</td>
                                <td>2016 청소년 정보윤리 창작음악제 참가안내</td>
                                <td>2016.08.08</td>
                            </tr>
                            <tr>
                                <td>123</td>
                                <td>하하하</td>
                                <td>2016 청소년 정보윤리 창작음악제 참가안내</td>
                                <td>2016.08.08</td>
                            </tr>
                            <tr>
                                <td>123</td>
                                <td>하하하</td>
                                <td>2016 청소년 정보윤리 창작음악제 참가안내</td>
                                <td>2016.08.08</td>
                            </tr>                                                            
                            <tr>
                                <td>123</td>
                                <td>하하하</td>
                                <td>2016 청소년 정보윤리 창작음악제 참가안내</td>
                                <td>2016.08.08</td>
                            </tr>     
                           <tr>
                                <td>123</td>
                                <td>하하하</td>
                                <td>2016 청소년 정보윤리 창작음악제 참가안내</td>
                                <td>2016.08.08</td>
                            </tr>  
                           <tr>
                                <td>123</td>
                                <td>하하하</td>
                                <td>2016 청소년 정보윤리 창작음악제 참가안내</td>
                                <td>2016.08.08</td>
                            </tr>  
                           <tr>
                                <td>123</td>
                                <td>하하하</td>
                                <td>2016 청소년 정보윤리 창작음악제 참가안내</td>
                                <td>2016.08.08</td>
                            </tr>                                                                     
                        </tbody>
                    </table>
                    <div class="paging">
                        <ul class="clearfix">
                            <li><a href="#">1</a></li>
                            <li><a href="#">2</a></li>
                            <li><a href="#">3</a></li>
                            <li><a href="#">4</a></li>
                            <li><a href="#">5</a></li>
                            <li><a href="#">6</a></li>
                            <li><a href="#">7</a></li>
                            <li><a href="#">8</a></li>
                            <li><a href="#">9</a></li>
                            <li><a href="#">10</a></li>
                            <li><a href="#">></a></li>
                            <li><a href="#">>></a></li>
                        </ul>                       
                    </div>

                </div> 
                
                                                                                                                                                                                                                                                                                                                                      
                
            </div>   <!-- border_box -->
            
            
            
            
            
            
        
        </div>
        

    </div>
    
    
    
  
 

    
    
   
    
    <? include_once("footer.php"); ?>
</body>
</html>