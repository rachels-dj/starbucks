...
?>
		...
		<form  action="./bupdate.php?bno=<?=$bno?>"  method="post"     enctype="multipart/form-data">
			<fieldset>
			<legend>글쓰기</legend>
			<p><label for="btitle">제목</label><input type="text" id="btitle" name="btitle" value="<?=$row['btitle']?>"/></p>
			<p><label for="bname">이름</label><input type="text" id="bname" name="bname" value="<?=$row['bname']?>"/></p>
			<p><label for="bemail">이메일</label><input type="email" id="bemail" name="bemail" value="<?=$row['bemail']?>"/></p>
			<p><label for="bpass">비밀번호</label><input type="password" id="bpass" name="bpass" /><span>* 수정, 삭제시 필수!</span></p>
			<p><label for="bcontent">내용</label><textarea id="bcontent" name="bcontent"  cols="60" rows="5"><?=$row['bcontent']?></textarea></p>
			<p><label for="bfilename">첨부파일</label><input type="file" id="bfilename" name="bfilename"/></p>
			<p  class="center"><input type="submit"  value="수정"/><input type="reset"  value="다시쓰기"/><input type="button"  value="목록"  onclick="history.go(-1);"/></p>
			</fieldset>
		</form>
	</div>
 </body>
